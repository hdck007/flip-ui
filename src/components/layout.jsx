  ...
  return (
    <div className="w-screen h-screen">
      ...
      <div className="md:flex w-full overflow-hidden md:overflow-auto">
        <div
          className={`w-[300px] h-screen top-0 bg-white absolute md:relative ${
            open ? "translate-x-0" : "translate-x-[-400px]"
          } md:translate-x-0 md:w-1/4 p-2 transition-all ease-in-out duration-300`}
        >
          <div className="w-full h-full md:shadow-lg md:border p-2">
            {open && (
              <span
                onClick={handleClick}
        ...
        <div className="w-full md:w-3/4 p-2">
          <div className="shadow-lg w-full h-full border">{props.children}</div>
        </div>
      </div>
    </div>
  );
